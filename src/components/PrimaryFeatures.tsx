...
function Feature({
  ...
  return (
    <div
      className={clsx(
        className,
        'relative overflow-hidden rounded-xl border border-2 p-6 shadow-md',
        isActive && 'border-slate-200 bg-white',
        !isActive &&
          'border-slate-200/30 opacity-75 transition-all duration-200 hover:opacity-100',
      )}
      {...props}
    >
      <div className="flex items-center gap-4">
        <div
          className={clsx(
            'w-9 rounded-lg',
            isActive ? 'bg-main-500' : 'bg-slate-500',
          )}
        >
        ...
        <h3
          className={clsx(
            'font-display text-lg font-[530]',
            isActive ? 'text-main-500' : 'text-slate-600',
          )}
        >
          {feature.name}
        </h3>
      </div>
      <p className="mt-4 font-sans text-base leading-6 text-slate-600">
        {feature.summary}
      </p>
    ...
    <div className="-mx-4 mt-16 flex flex-col gap-y-10 overflow-hidden px-4 sm:-mx-6 sm:px-6 lg:hidden">
      {features.map((feature) => (
        <div key={feature.summary}>
          <Feature feature={feature} className="mx-auto max-w-2xl" isActive mobile/>
...
function FeaturesDesktop() {
  return (
    <Tab.Group as="div" className="hidden lg:mt-16 lg:block">
      {({ selectedIndex }) => (
        <>
          <Tab.List className="mx-auto grid max-w-4xl grid-cols-3 gap-x-8">
            {features.map((feature, featureIndex) => (
              <Feature
                ...
                feature={{
                  ...feature,
                  name: (
                    <Tab ref={feature.ref} className="outline-none">
...
export function PrimaryFeatures() {
  features.forEach((feature, index) => {
    feature.ref = useRef<HTMLDivElement>(null)
  })
  return (
    <section
      id="primary-features"
      aria-label="Features for simplifying everyday business tasks"
      className="bg-slate-100/80 pb-14 pt-20 sm:pb-20 sm:pt-32 lg:pb-32"
    >
      <Container>
